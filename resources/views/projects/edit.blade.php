<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Project') }}: <span class="italic">{{ $project->name }}</span>
        </h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('projects.update', $project) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="budget" class="block mt-1 w-full" type="number" name="budget" :value="old('budget', $project->budget)" step="0.01" />
                            ...
                            <div>
                                ...
                                <select id="client_id" name="client_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}" @selected(old('client_id', $project->client_id) == $user->id)>
                                            {{ $user->name }}
                                        </option>
                                    @endforeach
                                </select>
